...
__device__ __host__ void clear_last_4_hex(BigInt* num) {
    // Last 5 hex digits = 20 bits
    // Clear the least significant 20 bits
    num->data[0] &= 0xFFFF0000;  // Keep upper 12 bits, clear lower 20 bits
}
...
__device__ BigInt* d_thread_keys;  // Array of starting keys for each thread
...
__global__ void start(const uint8_t* target, uint64_t iteration)
{
    ...
    // Early exit: Check if current key is within [min, max] range
    ...
    // --- Update this thread's key for next iteration: priv_base + BATCH_SIZE ---
    BigInt batch_offset;
    init_bigint(&batch_offset, BATCH_SIZE);
    
    BigInt next_key;
    ptx_u256Add(&next_key, &priv_base, &batch_offset);
...
bool run_with_quantum_data(const char* min, const char* max, const char* target, int blocks, int threads, int device_id) {
    ...
    printf("Starting persistent walking search...\n\n");
    ...
    while(true) {
        ...
        if (err == cudaSuccess) {
            ...
            if (elapsed_since_report >= 10.0) {
                ...
                printf("Progress: %llu iterations, %.2f MK checked, %.2f MK/s\n", 
                       iteration, total_keys_checked / 1000000.0, speed);
